...
| Headless fallback | Playwright + local Node.js project deps | Reliable frame capture when Puppeteer is unavailable but Playwright is installed |
...
### Headless Export Dependency Resolution

If `scripts/export-frames.js` fails because `puppeteer` is missing, check whether the repo already has `playwright` installed in local `node_modules`.

Important Node.js resolution pitfall:
- `require('playwright')` and `require('puppeteer')` resolve relative to the script location
- a helper script saved under `/tmp/...` may fail even when the dependency exists in the project
- save/run the exporter from inside the project directory (or otherwise ensure module resolution reaches the repo `node_modules`)

Reliable fallback:
- keep the sketch HTML anywhere
- save the frame-export helper inside the repo
- run it with `workdir` set to the repo root
- then stitch frames with ffmpeg into MP4

This is especially useful for quick vertical short-form videos where you want deterministic frame capture without setting up a larger web app pipeline.
...
7. **If Puppeteer is missing but Playwright exists** — use a small Playwright-based frame exporter from inside the project directory so Node can resolve local `node_modules`; scripts launched from `/tmp` may fail to `require('playwright')` or `require('puppeteer')`
8. **Iterative refinement** — edit the HTML file, user refreshes browser to see changes
9. **Load references on demand** — use `skill_view(name="p5js", file_path="references/...")` to load specific reference files as needed during implementation
